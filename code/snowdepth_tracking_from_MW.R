...
### RUBY START HERE ######
#setwd("../shrub_experiment/")
snow <- read.csv("data/imnaviat-snow-2012-2023.csv")
library(ggpubr)
library(tidyverse)
library(gghighlight)
library(plotly)
snow <- filter(snow, depth > -99.9)
tail(snow)

snow_plot <- ggplot(snow, aes(x = julian_day, y = depth*2.54, color = factor(year))) +
