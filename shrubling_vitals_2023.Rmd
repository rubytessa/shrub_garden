...
```{r}
## Files 
treatment_key <- read_sheet("https://docs.google.com/spreadsheets/d/1PL6d-5xT28oVSgbeNmw1YzwA9chrVwz8SPmr5KjDAag/edit#gid=0", col_types = "dddcDllc") %>% 
...
```{r}
# Sick Shrubs 
struggle_shrubs <- clean_data %>% filter(!is.na(Vitality)) %>% 
  select(Block.Plot, NewID, Vitality, everything())
...
```{r}
shrub_inventory <- clean_data %>%
  filter(is.na(Vitality))# filter(Length_max <= 50)

## TABLE BY TREATMENT
ggplot(shrub_inventory) + 
  geom_histogram(aes(x=Treatment, fill = Origin), stat = "count", position = "dodge") + 
